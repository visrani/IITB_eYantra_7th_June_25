class Monster {
    ...
    move() {
        ...
        const newX = this.x + (Math.random() < 0.5 ? dx : 0);
        const newY = this.y + (Math.random() < 0.5 ? dy : 0);
        ...
        if (newX >= 0 && newX < grid.width && newY >= 0 && newY < grid.height) {
            ...
            if (grid.getCell(this.x, this.y) === 'player') {
                if (window.powerUpActive || powerUpActive) {
                    // Player eats this ghost - remove this monster
                    const monsterIndex = window.monsters.findIndex(m => m === this);
                    if (monsterIndex !== -1) {
                        window.monsters.splice(monsterIndex, 1);
                        increaseScore(25, 'ghost');
                        createGhostEatenEffect(this.x, this.y);
                        console.log('Monster eaten by player during power-up!');
                        return; // Don't set cell as monster
                    }
                } else {
